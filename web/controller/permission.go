...
// requireFeature gates a route on something this platform can run at all (see
// service.Platform). The frontend never offers these where they are missing; this is
// the same answer for a direct API call.
func requireFeature(name string, available func(service.PlatformInfo) bool) gin.HandlerFunc {
	return func(c *gin.Context) {
		if !available(service.Platform()) {
			pureJsonMsg(c, http.StatusOK, false, name+" is not available on this platform")
			c.Abort()
			return
		}
		c.Next()
...
var (
	requirePanelUpdate = requireFeature("Panel update", func(p service.PlatformInfo) bool { return p.PanelUpdate })
	requireWarpCli     = requireFeature("warp-cli", func(p service.PlatformInfo) bool { return p.WarpCli })
)
