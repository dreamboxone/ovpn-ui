...
// charonBin / swanctlBin resolve the bundled launchers, else a host strongSwan package
// (charon is a libexec helper, never on PATH), else a bare name for PATH.
func charonBin() string {
	if p := backend.StrongswanBinPath("charon"); p != "" {
		return p
	}
	if p := backend.HostStrongswanBin("charon"); p != "" {
		return p
	}
	return "charon"
}
...
func swanctlBin() string {
	...
	if p := backend.HostStrongswanBin("swanctl"); p != "" {
		return p
	}
	return "swanctl"
}
...
func buildCharonConf() (string, error) {
	...
	b.WriteString(fmt.Sprintf("        include %s/*.conf\n", backend.StrongswanPluginConfDir()))
