...
// The inbound protocols the Add Inbound picker offers: every protocol, narrowed to what
// this host can serve when the server says so (PanelPlatform, from page/body_scripts).
// On OpenWrt that drops the protocols whose daemon or kernel module a router does not
// have, so the panel never offers an inbound the backend would refuse. Protocols itself
// stays whole: the rest of the model compares against its constants.
const InboundProtocolChoices = Object.freeze(Object.values(Protocols).filter((p) => {
    const allowed = typeof PanelPlatform !== 'undefined' ? PanelPlatform.inboundProtocols : null;
    return !Array.isArray(allowed) || allowed.length === 0 || allowed.includes(p);
}));
