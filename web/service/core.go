...
func daemonVersion(name string) string {
	...
	if p := backend.DaemonPath(name); p != "" {
		...
		bin = p
	} else if p := backend.HostStrongswanBin(name); p != "" {
		// A package-manager strongSwan (OpenWrt): charon is a libexec helper, so the
		// PATH lookup below would never find it and the version would read "—".
		bin = p
...
func (s *CoreService) GetCoresStatus() []CoreStatus {
	...
	return openwrtFilterStatus(all)
}
...
func (s *CoreService) ikev2Status() CoreStatus {
	...
	if !daemonInstalled("charon") && !backend.HasStrongswanBundle() && backend.HostStrongswanBin("charon") == "" {
		cs.State = CoreNotInstalled
...
func (s *CoreService) runProvisionSteps(emit func(ProvisionStep), cores []string) (rebootModules []string, rebootPkg string) {
	...
	// OpenWrt has its own package manager, kernel packaging and firewall, and none of
	// the distro steps below apply to it.
	if isOpenWrt() {
		s.runOpenwrtProvision(emit, selected, target)
		return nil, ""
	}
...
func (s *CoreService) StartProvision(cores []string) bool {
	...
	selected := openwrtFilterCores(validCoreNames(cores))
	if len(selected) == 0 {
		selected = installableCores()
	}
