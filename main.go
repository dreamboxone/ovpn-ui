...
func runWebServer() {
	...
	// On OpenWrt: record IKEv2 as ready when its packages are installed, and keep
	// OpenWrt's own strongSwan service from grabbing UDP 500/4500. No-op elsewhere.
	service.OpenwrtStartup()
...
func ensureMenuInstalled() {
	// The menu is a bash script driving systemd and the server installer. OpenWrt has
	// neither bash nor systemd; the ovpn-ui command and /etc/init.d/ovpn-ui cover it.
	if service.Platform().OpenWrt {
		return
	}
	dest := service.MenuScriptPath
