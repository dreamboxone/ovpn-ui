...
func coreMissingForProtocol(c *gin.Context, protocol model.Protocol) bool {
	// A protocol this platform cannot serve at all (an x86 daemon on a router) is not a
	// setup step away, so it gets its own refusal rather than a pointer to Core Settings.
	if !service.PlatformAllowsInbound(string(protocol)) {
		pureJsonMsg(c, http.StatusOK, false, "The "+string(protocol)+" protocol is not available on this platform")
		return true
	}
