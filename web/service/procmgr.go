...
func (p *managedProc) supervise(cmd *exec.Cmd, gen int) {
	...
	go func() {
		...
		p.terminateLocked()
		if p.launchLocked() == nil && p.name == ikev2ProcName {
			// charon keeps its connections, credentials and pools in memory only, loaded
			// over vici by swanctl. A respawned charon therefore comes back listening on
			// 500/4500 with nothing to match a client against and answers every IKE_SA_INIT
			// with NO_PROPOSAL_CHOSEN, until some unrelated inbound save happens to reload
			// it. reloadCharon retries while the new vici socket is not yet accepting.
			go func() {
				waitForPath(viciSocket, 10*time.Second)
				_ = reloadCharon()
			}()
		}
	}()
