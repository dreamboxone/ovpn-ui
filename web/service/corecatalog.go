...
func installableCores() []string {
	...
	// A router can serve only the cores its package feeds carry (see openwrt.go).
	return openwrtFilterCores(out)
}
...
func (s *CoreService) CoreCatalog() []CoreOption {
	...
	for _, c := range coreCatalog {
		// On OpenWrt, offering a core that cannot be installed there would only lead
		// to a setup run that does nothing. Built-in cores still run everywhere.
		if isOpenWrt() && !c.builtin && len(openwrtFilterCores([]string{c.name})) == 0 {
			continue
		}
		opt := CoreOption{
			Name:      c.name,
